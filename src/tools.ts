...
export function registerTools(
  ...
  server.tool(
    ...
    [
      ...
      "D-238: the response also carries pending_wakes — wake-marked messages (wake_priority set, any type) not yet acknowledged, same count+oldest-few shape and same orphan-sweep exclusion as pending_inbox. Delivery of a wake means board_ack; the WI still closes regardless (never blocking) — this just hands the closing agent the queue instead of relying on its memory of what pinged it.",
  ...
  server.tool(
    ...
    `Query doc_items in a project, or run a traceability check. ` +
      ...
      `Traceability mode: traceability='req_without_sdes' (REQ rows with no linked SDES), 'sdes_without_uat' (downstream chain), 'req_without_origin' (D-206 upstream axis — REQ rows with no admitted origin), or 'broken_refs' (link integrity — see below). ` +
      ...
      `Example (link integrity): doc_query({project_id:"<uuid>", traceability:"broken_refs"}).`,
    {
      project_id: z.string().uuid().describe("Project scope — mandatory"),
      ...
      traceability: z.enum(["req_without_sdes", "sdes_without_uat", "req_without_origin", "broken_refs"]).optional().describe("Run a traceability gap check instead of a plain filter"),
