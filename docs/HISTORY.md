...
**Costruito:** (1) `gtd_add` (`src/tools.ts`) stampa `created_by_wi_id` con l'id del WI ATTIVO del CHIAMANTE (`selfSlug`, non `targetOwner` — è il WI che sta eseguendo la creazione a contare, non l'owner del nuovo item, per coprire anche il caso loomy/broker che crea un item per un altro agente durante il proprio WI) — stessa query di `checkAutopilotArmGuard` (`loomx_work_items` filtrato `agent_slug`+`status=active`), best-effort: un fallimento della lookup logga su stderr e non blocca mai la creazione dell'item. Aggiunto `created_by_wi_id` alla `.select()` di risposta per trasparenza. (2) `wi_end` (`src/wi.ts`): nuova `fetchGtdFollowOnsSafe` — query diretta `loomx_items WHERE created_by_wi_id=<wi_id>` (non serve `runDoc`/RLS: `loomx_items` è una tabella board, non dietro `doc_rw` come `doc_item_wi_links`), stesso contratto non-bloccante (try/catch, log stderr) di `fetchWiDocLinksSafe`. Se non vuota, appende una terza entry `{auto:true, executed:true, scheduled_at, payload:{type:"gtd_follow_on", items, count}}` a `side_effects_log`, sullo stesso pattern delle fonti 1 (`files_touched`) e 2 (`doc_item_wi_links`) — mai un'entry per una fonte vuota.

**Verifiche.** `tsc`/`npm run build` puliti, `npm test` **438/438** (+2 nuovi su `wiEnd`: entry `gtd_follow_on` su chiusura `done` con più follow-on filtrati correttamente per `wi_id` — un item con `created_by_wi_id` di un altro WI resta escluso — e su chiusura non-`done` `waiting`). **Live, non solo per lettura del codice** (script throwaway `tests/verify-created-by-wi-id-tmp.ts`, cancellato dopo l'uso): confermato che (a) la query di lookup del WI attivo di `gtd_add` trova davvero il WI di questa sessione, (b) un INSERT reale con `created_by_wi_id` valorizzato round-trippa esatto (nessun cast/troncamento a sorpresa sulla colonna appena applicata), (c) la query di lettura di `wi_end` (`.eq("created_by_wi_id", wiId)`) la ritrova. Cleanup previsto (DELETE della riga probe) fallito con `permission denied` — il ruolo nativo `board-mcp` non ha DELETE su `loomx_items` (nessun tool lo espone, per design: soft-delete a `gtd_status=trash` è l'unico "cancellato" di questo sistema). La riga probe resta quindi in `trash` (titolo esplicito `[probe] ... — safe to ignore/delete`), innocua e invisibile a `gtd_inbox`/`gtd_query` di default.

**Decisioni prese:** nessuna nuova decisione formale — implementazione tecnica dello scope già confermato da it-manager (msg `9a7729df`) nella sessione #184; CLAUDE.md aggiornato (sezione `wi_end` "Deviations", punto 2) con l'accounting completo al posto della nota "terza fonte NON costruita". `v0.32.13` bumpato in `package.json`.
**Blocchi / note:** nessuno. Non deployato via commit a flotta — le altre window restano sul dist precedente finché non riavviano (G4); questa stessa finestra è partita col build precedente, quindi la verifica applicativa è passata dai test unitari + dal source diretto (script live), non dai tool MCP connessi.
**Prossima sessione:** nessun follow-on aperto. GTD `8332ff1b` chiuso.
...
**Task 2 — costruzione `agent_context()` (msg frame `f7ff99d9`, SDES-001, progetto frame-method-as-service, R di board-mcp da RACI SoW DEL-000).** SDES-001 completo NON leggibile da questa identità (`doc_item_resolve`/`doc_query` → confidentiality error D-015: board-mcp non ha `loomx_project_members` sul progetto, verificato via `loomx_agent_in_project` — `agent_id='frame'`, nessun team condiviso). Segnalato a frame (msg `c9d8d991`), costruito dal contratto riassunto nel messaggio (D-136 §5), verifica alla consegna richiesta esplicitamente.

**Costruito:** `src/agentContext.ts` — nuovo tool `agent_context()` senza parametri. `role` = `buildRoleCard()`, estratta dal branch "card" di `org_lookup` e riusata da entrambi (un'unica implementazione, non una copia — `org_lookup` ora delega). `constitution` = una sola RPC `gov.applicable_norms(p_agent)`, fail-open dichiarato (`constitution_unavailable`) finché dba non la pubblica. `work` = WI attivo (colonne leggere, niente JSONB pesanti), GTD armati/`next_action` top 5 (fetch bounded 100 + filtro/ordina in JS — mai `.or()` PostgREST-style contro il pg-shim, cautela D-119 sul booleano-vs-parametro-testuale non verificato), `pending_inbox`/`pending_wakes` (D-205/D-238, self-close by construction). `payload_version: "0"`, nessun body libero.

**Fix collegato, scoperto costruendo:** `PgShimClient.rpc()` (`src/pg-shim.ts`) rifiutava a monte ogni nome funzione con un punto (`^[a-zA-Z_][a-zA-Z0-9_]*$`) — `gov.applicable_norms` non avrebbe mai raggiunto il DB. Ora riusa `ident()` (lo stesso helper già fidato per `.from("gov.*")`), quotato per segmento; il match del risultato scalare usa il nome non qualificato (Postgres nomina la colonna di ritorno `applicable_norms`, non `gov.applicable_norms`). Verificato dal vivo: l'errore ora è `function gov.applicable_norms(p_agent => unknown) does not exist` (42883, DB raggiunto), non più l'errore di validazione client-side.

**Verificato dal vivo, non solo unit test** (`tests/verify-agent-context.ts`, identità board-mcp reale): ruolo identico a `org_lookup(agent=self,question="card")` (parità asserita, stessa funzione); `constitution` fail-open confermato contro il DB reale; `work.active_wi` mostra il WI di questa stessa sessione; `gtd_top` 5 righe reali, ordinate e filtrate correttamente; `pending_inbox`/`pending_wakes` mostrano il wake di frame ancora da ackare. 13 nuovi test unitari (`tests/agentContext.test.ts` 8, `tests/pgshim-rpc.test.ts` 5) — 398/398 verdi (era 385). `tsc`/`npm run build` puliti. `./agentContext.js` aggiunto al preload G4 (`server.ts`). v0.32.2 → v0.32.3.

**Consegna:** `done` a frame in coda (test count + payload d'esempio), con verifica esplicita richiesta sui campi che non potevo confermare dal disegno (campi di assenza REQ-026 esatti).

**Decisioni prese:** nessuna (entrambi i task: mandato esplicito "nessuna decisione" / build su contratto altrui).
**Blocchi / note:** `db_writes` senza segnale indipendente (GTD `acb26106`, non armata) resta aperto lato auditor/loomy. SDES-001 pieno resta non leggibile da board-mcp — se serve lettura ricorrente, membership `loomx_project_members` da dba (non richiesta qui, solo segnalata).
**Prossima sessione:** nessun follow-on aperto lato build; entrambi i thread restano `waiting` su loomy/frame.
