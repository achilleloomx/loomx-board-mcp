...
42 tool base esposti a ogni agente (24 board/gtd/wi/runtime/ping + 9 doc_* document model + 5 subscription tool + 3 staleness/decay tool + 1 `org_lookup`) + 8 tool home_* (condizionali, richiedono HOME_FAMILY_ID + HOME_USER_ID):
...
4/4 tool ratificati in D-186 live (design SDES-SUB-000..007, doc `714d3313`, progetto board-mcp `596cd5fc`), **+1 `doc_repoint`** (v0.24.2, UAT-GOV-029 — non in D-186, nasce dalla migrazione dba `20260828065000`). Girano sotto `doc_rw` come i `doc_*` (stesso `runDocRw`, `src/subscriptions.ts`).
