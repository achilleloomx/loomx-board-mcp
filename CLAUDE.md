...
> **Gate:** `tests/strict-tool-args.test.ts` verifica che **ogni** tool registrato rifiuti una chiave ignota nominandola (72/72) → build rossa se un tool rientra loose. Il gate registra anche gli 8 `home_*` condizionali (setta `HOME_FAMILY_ID`/`HOME_USER_ID`): senza, girerebbe su 59 e passerebbe lasciando fuori 13 tool.
