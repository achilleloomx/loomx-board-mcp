...
49 tool base esposti a ogni agente (24 board/gtd/wi/runtime/ping + 13 doc_* document model — incl. `doc_rename` e `doc_structure`, v0.25.0, e `doc_item_retire`/`project_retire`, v0.32.6 — + 7 subscription tool — incl. `doc_fact_sync` — + 3 staleness/decay tool + 1 `org_lookup` + 1 `agent_context`) + 8 tool home_* (condizionali, richiedono HOME_FAMILY_ID + HOME_USER_ID):
...
> **Dove sta il fix:** `src/strictTools.ts` — `withStrictToolArgs()` avvolge il server e irrigidisce il **punto di registrazione**, applicato in `registerTools` (`tools.ts`) e `registerHumanTools` (`humanTools.ts`). Zero diff sui call site, e un tool aggiunto domani nasce strict per costruzione. Il perimetro reale era **72** registrazioni al momento del fix (ora 74 con `doc_item_retire`/`project_retire`, v0.32.6), non 67: i 5 di `humanTools.ts` non erano nel conteggio segnalato.
> **Il contratto pubblicato non cambia** (misurato, `tests/verify-strict-contract.ts`): 66 schemi su 67 identici byte per byte. Unica eccezione dichiarata: `home_grocery_categories`, registrato con `{}`, che ora pubblica `additionalProperties:false` dove prima non dichiarava nulla e accettava qualunque chiave.
> **Gate:** `tests/strict-tool-args.test.ts` verifica che **ogni** tool registrato rifiuti una chiave ignota nominandola (dinamico su tutti i tool registrati — 72/72 al momento del fix, 74/74 dopo v0.32.6) → build rossa se un tool rientra loose. Il gate registra anche gli 8 `home_*` condizionali (setta `HOME_FAMILY_ID`/`HOME_USER_ID`): senza, girerebbe su 59 e passerebbe lasciando fuori 13 tool.
...
|---|---|---|
| `board_send` | Invia messaggio con summary e tags opzionali. **`wake_priority?`** (D-093, v0.13.0): normal\|high\|urgent — marca il messaggio per cold-wake. **`requested_model?`** (D-098/D-101, v0.16.0): modello per il lancio del destinatario sul cold-wake (es. `sonnet`, `opus`) — significativo solo insieme a `wake_priority`, scritto su `board_messages.requested_model`, free-text pass-through (nessun default/normalizzazione lato tool). **`auto_gtd?`** (GTD 994b3bbc): crea anche un GTD per il destinatario (owner=recipient, source='board', source_ref=id messaggio), deduplicato su re-invii — default false, non cambia il comportamento esistente. **Destinatario umano → GTD forzato (IA-009/CORE-019, v0.32.8, GTD 72436ca3):** se `to_agent` è un umano (`loomx_role_cards.human_ref` combacia con lo slug, case-insensitive — segnale DB-driven: un valore usato come `human_ref` da altre schede è per costruzione una persona, mai un agente con propria scheda), il GTD viene creato **a prescindere** da `auto_gtd` (`shouldForceAutoGtd`, `src/tools.ts`) — nessuno interroga `board_inbox` per una persona, quindi senza forzatura il messaggio atterrerebbe inosservato. Risposta porta `auto_gtd_forced` quando scatta. Verificato dal vivo (`tests/verify-ia009-human-recipient.ts`): oggi l'unico destinatario umano registrato è `Achille` (`board_agents.slug`, agent_code `000`, nessuna riga in `loomx_role_cards` come scheda propria). **Non copre** `board_broadcast` (stesso gap, follow-on) né la registrazione di un umano mai visto prima (richiede una riga `board_agents`, D-005 — fuori dal perimetro di un tool-layer). **Hint cold-recipient (D-118 c2, v0.15.0):** se `type∈{task,question,blocker}` e `wake_priority` omesso e il destinatario risulta cold (`loomx_agent_runtime.heartbeat_at` stale/assente, soglia 10'), la risposta include `hint` — mai un errore, il send va comunque a buon fine. Gated da `LOOMX_RW_GUARDS_ENABLED` (default off: log-only su stderr, nessun campo in risposta finché il flag non è `1` — eval-first, vedi HISTORY). **`signed_norm?`/`signed_norm_project_id?` (CORE-019 inv.3/CORE-020, v0.32.10, GTD 0ba9afa6):** coppia obbligatoria (mai una sola) — norma che autorizza il messaggio, validata via `doc_item_resolve` reale PRIMA dell'INSERT. Accettata solo quando `to_agent` è un destinatario umano (stesso predicato `isHumanRecipient` di IA-009), altrimenti rifiuto esplicito. Scritta nello stesso INSERT (`board_messages.signed_norm`; `from_agent` copre già l'ID). Risposta porta `signed:{signed_by,signed_norm}` quando scatta. **Non copre** le notifiche di sistema automatiche (`project_retire`'s subscriber notify, l'escalation message di `wi_end` D-135) né `ping`/`board_broadcast` — scelta deliberata (D-136 §5): sono notifiche automatiche, non un atto di delega deciso da un agente, quindi non rientrano in CORE-019 inv.3 | INSERT (from_agent = self) [+ INSERT loomx_items se auto_gtd] |
...
| `gtd_update` | Aggiorna item esistente (owner-only, loomy puo' tutto). **D-069 guard:** rifiuta `autopilot=true` se l'owner ha un WI active (two-phase arm). **D-093 hardening:** il broker (loomy-assistant) puo' armare `autopilot=true` su un item di un altro owner SOLO se `clarified_at IS NULL` (mai ackato dall'owner); se `clarified_at` e' valorizzato deve escalare a loomy — stessa logica del gate `clarified_at` (non settabile dal broker su item altrui), ma condizionale invece di esclusione totale. **`signed_norm?`/`signed_norm_project_id?` (CORE-019 inv.3/CORE-020, v0.32.10, GTD 0ba9afa6):** coppia obbligatoria — norma che autorizza la scrittura, validata via `doc_item_resolve` reale PRIMA dello UPDATE. Accettata solo quando la chiamata è cross-owner (loomy/broker su item non proprio) E l'owner (post-update) è un destinatario umano (`isHumanRecipient`) — altrimenti rifiuto esplicito. `signed_by` è sempre `selfSlug`, mai accettato dal chiamante. Risposta porta `signed:{signed_by,signed_norm}` quando scatta | UPDATE |
| `gtd_query` | Query flessibile — **`preview_only=true` default**, limit 20. **Cross-owner read (`project_id` branch):** oltre a loomy/broker (sempre), il responsabile del progetto interrogato (`loomx_projects.agent_id === selfSlug`) legge cross-owner le righe scoped a quel SOLO `project_id` (DEC-002, v0.32.2, msg loomy 565931ec — "si allarga una lettura, mai una scrittura": `gtd_add(owner)`/`gtd_update(owner)` restano loomy-only). Funzione pura `gtdQueryProjectCrossOwnerRead`, `src/tools.ts` | SELECT + JOIN |
...
| `project_list` | Lista progetti (id, name, short_name, status, agent_id, `is_sandbox`, **`container_type`/`is_critical`** — v0.33.1, T2 minimo 7/9, dba msg 85c8441e, SDES-ID-005; filtro opzionale `container_type`, uguaglianza esatta; colonne read-only per board-mcp, le scrive solo loomy via trigger dba) — no write path, per scoprire project_id senza Management API. **`is_sandbox` (v0.22.2, DEL-003/SDES-009, msg forge 162add27):** i progetti sandbox sono esclusi per difetto dai conteggi (`is_sandbox=false` implicito) — `include_sandbox=true` per vederli | SELECT loomx_projects |
...
2. `side_effects_pending` — il design menziona array pending separato; lo schema ha un solo `side_effects_log JSONB` (D-032). Le entry pending vengono scritte come `{pending: true, scheduled_at, payload}`; la skill session-manager v2 le convertirà in `{executed: true, ...}`. **Auto-derivazione (v0.32.11, it-manager msg 9a7729df/3ad5da5a, "strada 2" — GTD register agent-issue-tracker 2c437678):** `side_effects_log` risultava vuoto su 51/51 chiusure reali perché nulla popolava mai `side_effects_pending`. Ora `wi_end` APPENDE, su ogni percorso di chiusura, entry `{auto:true, executed:true, scheduled_at, payload:{type,...}}` derivate da dati già posseduti dal motore — mai richieste al chiamante: `files_touched` (da `in_flight_state`, accumulato via `wi_checkpoint`) e `doc_item_wi_links` creati durante il WI (riuso della query del gate D-074 dove già eseguita, query leggera non-bloccante altrove). Mai un'entry per una fonte vuota (niente zero finto). **Terza fonte costruita (v0.32.13, GTD 8332ff1b, dba msg 0f3d05e5 — colonna `loomx_items.created_by_wi_id` nullable, FK `loomx_work_items(id)` ON DELETE SET NULL, applicata in produzione su entrambi i lati):** `gtd_add` stampa `created_by_wi_id` con l'ID del WI ATTIVO del chiamante (`selfSlug`, non l'`owner` del nuovo item — è il WI che sta eseguendo la creazione, non quello di chi la riceverà) quando presente, best-effort (una lookup fallita non blocca mai la creazione dell'item). `wi_end` legge `loomx_items WHERE created_by_wi_id=<wi_id>` (query diretta, `loomx_items` non è dietro RLS/doc_rw come `doc_item_wi_links` — nessun `runDoc` necessario) e appende una terza entry `{type:"gtd_follow_on", items, count}` quando non vuota. Non confondere con `blocks_wi`, colonna preesistente sulla stessa tabella verso lo stesso target ma con semantica diversa (dba, stesso msg).
...
>
> **Contratto cambio-modello (D-101, v0.16.0):** `request=model` è legittimo in QUALSIASI mode (interactive/autopilot/chat/dream), non solo autopilot — bug storico 69f39997 era un CHECK constraint DB mancante `'model'` nell'enum, già fixato dal DBA il 2026-07-21, verificato live. Guard aggiuntivi via `LOOMX_MODEL_GUARDS_ENABLED` (default **ON** dal v0.32.12, CP-2/CV-6 — vedi sotto; escape hatch `=0` per spegnerli): Haiku **mai** ammesso quando la riga target ha `mode=autopilot` (§0quater) → rifiutato esplicitamente; `cost_notice` (mai bloccante) su upgrade di tier verso un modello più costoso. `requested_model` resta free-text pass-through (nessun registro alias canonico — scelta DBA, vedi D-101) — niente normalizzazione, niente validazione di esistenza.
>
> **Accensione guardia modello (v0.32.12, CP-2/CV-6, registro `loomx-ai-governance` doc `34aef3d4`, loomy msg `1bd39554`, 2026-09-16):** `modelGuardsEnabled()` era gated OFF-by-default dietro il mandato eval-first D-118 ("niente flip senza suite verde"). CV-6 ha misurato "i dodici collaudi del risveglio" (21 codici/141 controlli/zero fallimenti) — un catalogo DIVERSO da `E2E-MODEL-*` (loomx_evals), che questo flag documenta esplicitamente come proprio gate. Verificato prima di flippare: i **soli due** call site che questo flag governa (`src/tools.ts`, handler `runtime_request`) sono il rifiuto Haiku-in-autopilot e il cost-notice — entrambi hanno verdict `pass` già registrato (`E2E-MODEL-06`/`E2E-MODEL-08`, riverificato su HEAD `a19da81` via `tests/model-switch-guards.test.ts`). Nessuno dei gap residui del catalogo più ampio tocca questi due comportamenti: `E2E-MODEL-05` (propagazione lato reconciler di `requested_model`, `not_run`) e `E2E-MODEL-01/04/07/09` (lato it-manager, mai confermati qui) sono estranei; `E2E-MODEL-03/10` (alias↔ID, modello deprecato) sono esclusi per scelta esplicita dba (`requested_model` free-text by design), non un fallimento pendente. Flip **non retroattivo/non distruttivo**: il blocco Haiku impedisce solo un FUTURO switch-a-haiku in autopilot (nessun effetto su un agente già in esecuzione su Haiku), il cost-notice è puro avviso mai bloccante.
...
| Tool | Descrizione | Operazione DB |
|---|---|---|
| `org_lookup` | `agent?` → role-card + archi (reports_to/escalates_to/asks_help_from); `question` seleziona `card`\|`chain`\|`escalation`\|`help` (default `card`); `domain?` filtra escalation/help; `project?` → matrice RACI — **riferimento robusto (v0.33.1, SDES-ID-005, dba msg 85c8441e):** UUID, prefisso ≥8 hex, `short_name` (esatto poi case-insensitive), `name` (case-insensitive); prima corrispondenza unica vince (`matched_by` in risposta), ambiguità = errore con candidati, mai una scelta (`src/projectRef.ts`). Limite: un vecchio `short_name` rinominato non risolve finché dba non crea `loomx_project_aliases`; `sow?` filtra per SoW (WIP); `raci?` filtra la matrice per ruolo | SELECT loomx_role_cards / loomx_org_edges / loomx_sow_raci |
...
### T2 minimo — agent_context v1, sessione a epoche, `board-cli`, `wi_start` a differenza (v0.33.0, SDES-001 v1 / SDES-005 v1, task frame `1a7ed39d`, delega GM-001)

> **Supera la sezione sopra dove divergono:** `payload_version:"1"`; `constitution` è deprecata un ciclo (`{deprecated:true, see:"critical_core"}`).

| Pezzo | Dove | Cosa fa |
|---|---|---|
| `agent_context()` v1 | `src/agentContext.ts` | `critical_core {norms[{code,title,summary,version,grade,grade_source,source}], critical_threads, critical_core_unpublished[], critical_registry_empty, core_bytes}` dalla sola RPC a parametri NULL; `session {id, epoch}`. Fail-open: `critical_core:null + critical_core_unavailable`, `session:null + session_unavailable`. `rpc_contract_version:"v0"` **dichiarato** finché la RPC v1 di dba non è applicata (mai letto come «v1 dice vuoto») |
| `board-cli agent-context [--compact] [--session-id <id>] [--trigger startup\|resume\|clear\|compact]` | `src/cli.ts` → `dist/cli.js` (`bin` in package.json) | Per l'hook `SessionStart`. **Ogni esecuzione apre una nuova epoca** (`gov.session_epoch_open`) e registra il consegnato. Stessa identità nativa del server (`resolveSelfSlug`; senza `--agent`: ruolo DB nativo, cross-check `LOOMX_AGENT_SLUG`), stesso modulo. `session_id`/`source` letti anche dal JSON dell'hook su stdin se i flag mancano. Env: `DATABASE_URL`/`DOC_RW_DATABASE_URL`, con ripiego su `LOOMX_DB_URL`/`LOOMX_DOC_RW_URL` (le stesse che `.mcp.json` mappa). **Mai bloccante:** qualunque errore → una riga d'avviso su stdout, exit 0; watchdog 4,5 s (l'hook ha timeout 5 s). Misurato: 0,5 s |
| Registro di sessione | `src/sessionNorms.ts` | **Unico** punto di chiamata della RPC delle norme (REQ-009/UAT-014: `grep -rn applicable_norms src/` → 1 riga) + `SessionStore` (scritture sotto `doc_rw`, contratto dba msg `af4c0e19`: epoche solo via funzione, `agent_code` derivato e riletto dalla riga d'epoca, `source_project_id` in chiave, `version` dentro la PK di `gov.session_norms` insert-only, `form ∈ {compact, full}`). Chiave di una Decisione = `(source.project_id, code)` — **mai** lo slug (i Thread si rinominano) né il codice nudo (D-167) |
| `wi_start` | `src/wi.ts` + `prepareWiNorms`/`commitWiNorms` | progetto dal GTD legato (`loomx_item_projects`; nessuno → `houses_unresolved:["<no project>"]`, soli critici; >1 → il primo per id, **dichiarato** in `norms_project_note` — l'unione è lavoro della RPC, mai fatta in TS) → RPC → differenza con l'epoca → `delivered[]` / `already_in_session[]` (soli codici; qualificati `origine:codice` **solo** se omonimi nel set) / `critical_core_delivered_at_wi_start` / `houses_unresolved` / `critical_core_unpublished` / `critical_registry_empty` / `list_bytes` / `norms_session`. Scrive il **set dovuto intero** in `gov.wi_norms` e i consegnati in `gov.session_norms`. RPC giù → `norms_unavailable`, il WI si apre; registro giù → consegna **tutto** + `norms_registry_warning` (sovra-consegnare è la direzione sicura). Solo per il **proprio** WI (loomy che apre per un altro: niente norme, la sessione è del chiamante) |
| `wi_resume` | `src/wi.ts` | **Non** riscrive `gov.wi_norms`. L'epoca contro cui si è calcolata la differenza sta in `in_flight_state.norms_session`: stessa epoca → `norms_epoch_unchanged:true`; epoca cambiata → differenza ricalcolata e riconsegnata |
| Flag | `src/flags.ts` | `LOOMX_WI_NORMS=off\|pilots\|all` (default `pilots`; valore non riconosciuto = off: un refuso non allarga mai un rollout) + `LOOMX_WI_NORMS_PILOTS` (default `dev-frame,acme-lab,analyst-pieroni`). **Agente non coperto: `wi_start`/`wi_resume` identici a prima** — nessuna RPC, nessuna chiave in più, nessuna scrittura (testato). Fase dura REQ-032: `LOOMX_WI_NORMS_HARD_SLUGS` (default vuoto = spenta) → rifiuto `E_CRITICAL_CORE_MISSING` **prima** di qualunque scrittura (nessun GTD/WI a metà); registro illeggibile → resta grazia, dichiarata |

**Deviazione dichiarata da SDES-005** («dopo l'INSERT del WI»): il calcolo (RPC + differenza) avviene **prima** dell'INSERT, le due scritture che richiedono `wi_id` dopo. Serve alla fase dura: un rifiuto non deve lasciare un WI aperto. Stesso esito osservabile in grazia.

**Come il processo stdio sa quale sessione serve** (scelta board-mcp, `resolveCurrentSession`). Misurato 2026-09-19 su Claude Code 2.1.277: il server MCP è figlio **diretto** di Claude (`process.ppid`) e l'ambiente dell'hook porta `CLAUDE_PID` — stesso valore, stabile oltre `/clear` (dove il `session_id` cambia). `CLAUDE_CODE_SESSION_ID` nell'env MCP è congelato allo spawn: usato solo per **dare un nome** a una sessione implicita, mai per cercarne una. Ordine: (1) ultima epoca propria, aperta dopo il boot del processo (−120 s di tolleranza), con `host_pid == ppid`; (2) altrimenti ultima con `host_pid NULL`; un'epoca con `host_pid` **diverso** è un'altra sessione concorrente dello stesso agente e non viene **mai** adottata; (3) altrimenti il processo apre da sé un'epoca `mcp_implicit` (agente senza hook o hook fallito: la grazia REQ-032 ha bisogno di un posto dove registrare). **Limite dichiarato:** due sessioni concorrenti entrambe con `host_pid NULL` → la regola 2 può adottare l'epoca della sorella, e una Decisione non critica ricevuta da lei risulta `already_in_session` qui; `gov.wi_norms` conserva comunque il set dovuto intero, quindi l'audit lo vede.

**pg-shim:** due opt-in per chiamata, default invariati — `insert/upsert(..., {noSyntheticId:true})` (le tabelle `gov.session_*`/`gov.wi_norms` non hanno colonna `id`: in no-RETURNING mode lo shim ne iniettava una) e `upsert(..., {ignoreDuplicates:true})` → `ON CONFLICT DO NOTHING`. `DocRwDb.sessionEpochOpen` = entry nominata per `gov.session_epoch_open` (nessun `.rpc()` generico, stessa disciplina del resto dell'interfaccia).

**Post-apply (2026-09-19, dba msg `85c8441e`, v0.33.1):** migrazione dba applicata. Misurato dal vivo sulla sessione board-mcp (non pilota): epoca 1 aperta come `mcp_implicit` (host_pid = ppid — l'hook `SessionStart` con `board-cli` non è ancora cablato in questo repo), `rpc_contract_version:"v1"`, 39 righe in `gov.session_norms` sotto RLS `doc_rw`, `form=full`; `gov.wi_norms` = 0 righe sul WI, atteso (board-mcp fuori da `LOOMX_WI_NORMS_PILOTS`). **Misura sui piloti (2026-09-19, loomy msg `e2530af8`, `tests/verify-t2-pilots.ts`, 55/55, ogni pilota in una transazione `doc_rw` annullata, zero residui):** moduli reali sotto lo slug del pilota. Nucleo critico: 39 Decisioni, 9 Thread, 3 non pubblicate, 14 147 B full / 3 670 B compact, `rpc_contract_version:"v1"`. Dovute per progetto: dev-frame 58, acme-lab 58, analyst-pieroni 101, `houses_unresolved:[]`. **Con hook** (epoca `startup` + nucleo registrato `full`): `wi_start` consegna solo la differenza (19/19/62 righe, 11–28 KB), nucleo mai riconsegnato, secondo `wi_start` nella stessa epoca → 0. **Senza hook** (epoca `mcp_implicit`, grazia REQ-032): consegna tutto (58/58/101, **25–42 KB** in risposta a `wi_start`). `gov.session_norms` insert + `ON CONFLICT DO NOTHING` sotto RLS verificati; `gov.wi_norms` verificato sul WI proprio di board-mcp (39/39, idempotente) e **rifiutato dalla RLS** (`gov.wi_is_callers`) su un WI non del pilota. Limite della misura: il ruolo nativo board-mcp non legge WI/GTD altrui, quindi il progetto del pilota è passato da `loomx_projects` e non da un suo GTD reale. **Nessun repo pilota ha oggi l'hook `SessionStart` con `board-cli`** → al primo `wi_start` reale i piloti prendono il ramo senza hook.

**Stato alla consegna del codice (2026-09-19):** migrazione dba `20260919013000_gov_t2_minimo_…` **staged, non applicata** (attende l'autorizzazione di loomy). Verificato dal vivo col dato che c'è: `board-cli` risponde in fail-open (`sessione non registrata: function gov.session_epoch_open … does not exist`, RPC ancora v0), exit 0 anche a DB irraggiungibile e a trigger invalido. **Non ancora misurato dal vivo:** scritture reali in `gov.session_norms`/`gov.wi_norms` e `INSERT … ON CONFLICT DO NOTHING` sotto le policy RLS di dba — dovuto dopo l'apply (GTD follow-on).
...
| `doc_link` | UUID-only, enum `target_kind doc\|gtd\|wi` instrada alle 3 tabelle (nessun param code). **Rilettura D-132 estesa a tutti e 4 gli INSERT (2026-09-03, GTD `b4e07ba6`):** ciascuno dei 4 percorsi (`doc_item_gtd_links`, `doc_item_wi_links`, `doc_item_links` intra-progetto, `doc_item_xproject_links` cross-progetto) è riletto e confrontato prima di rispondere `ok` — stessa classe D-133 già chiusa su `doc_create`/`doc_supersede`, qui misurata prima di armare: la migration dba `20260816110000` ha tolto UPDATE da `doc_rw` su quelle 4 tabelle ma non SELECT/INSERT, e la rilettura post-INSERT è stata verificata dal vivo (`tests/verify-doc-link-reread.ts`, transazione annullata) prima di scrivere il codice, non assunta. **Hook `fact_subscription` (v0.26.0, D-225/4bis, SDES-DECAY-003):** su un legame `verifies`/`satisfies` la risposta dichiara sempre cosa è successo al decadimento — derivata (il progetto aveva già l'opt-in: il legame porta decadimento dall'istante in cui nasce), oppure **perché no**: progetto senza opt-in (il legame è dichiarato ma non fa decadere nulla — la forma esatta di REG-011, detta invece che taciuta), cross-progetto (mai derivata: nessun pin di versione lì, debito R6), ammissione sospesa, già coperta, opt-in non verificabile (**si astiene** — una scrittura irreversibile non si fa su una precondizione non verificata). **Mai bloccante e mai distruttivo:** sotto `doc_rw` la chiamata è una sola transazione, quindi l'hook gira dentro un `SAVEPOINT` — senza, una sua query fallita aborterebbe l'INSERT del link (misurato dal vivo il 2026-08-29) | INSERT doc_item_links / doc_item_gtd_links / doc_item_wi_links [+ INSERT gov.doc_subscriptions se derivabile] |
| `doc_link_by_code` | Sugar resolve+resolve+link project-scoped (stesso resolver loggato) | SELECT×2 + INSERT |
| `doc_supersede` | Old→superseded (immutabile) + nuova riga + edge `supersedes`; codice trasportato. **Rilettura D-132 estesa (v0.30.1, GTD 8b97b1ca):** i tre write diretti su `doc_items` (detach del code dalla vecchia riga, INSERT della nuova versione, mark `status='superseded'`) sono ciascuno riletti e confrontati prima di procedere — prima il mark si "accorgeva per rimbalzo" solo quando `gov.relink_superseded` rifiutava con 23514 (precondizione della funzione, non un controllo dedicato); ora ciascuno dei tre fallisce esplicito, PRIMA che il passo successivo tratti una scrittura silenziosamente non atterrata come reale | UPDATE + INSERT×2 |
| `doc_item_retire` | (v0.32.6/v0.32.7, SDES-DOCM-026, Ritiro progetto fase 3, GTD `1061ce6e`) Il caso che `doc_supersede` non copre: fine deliberata, senza erede. Scrive `attrs.no_successor_reason=reason` (obbligatorio) sulla STESSA update che il guard terminale legge come propria via di fuga dichiarata — non introduce un meccanismo nuovo, usa quello già esistente. **Non blocca** su link/xproject_link/sottoscrizioni in ingresso (bloccare vanificherebbe il suo scopo di override deliberato) — li ritorna in `incoming` + `notify` (una entry per sottoscrizione attiva toccata, owner risolto) perché il chiamante possa avvisare (REQ-DOCM-027); una chiamata diretta non invia nulla da sola, lo fa `project_retire`. **Schema LIVE (dba msg `9c2add8d`, 2026-09-16, 4 migrazioni):** `doc_items_status_check` ammette `'retired'`. **Correzione al disegno originale (dba, stessa msg):** il set di stati terminali NON è cablato nel trigger `gov.doc_items_require_successor_on_terminal` — la funzione legge il parametro di governance `docm_m5_terminal_statuses` via `gov.doc_m5_terminal_statuses()` (il letterale `superseded/deprecated/archived` è solo il SUO fallback); dba ha aggiornato il parametro con `gov.param_set`, includendo `'retired'` ed escludendo deliberatamente `'rejected'` ("ritira una proposta mai adottata" è un errore di categoria). `fetchTerminalStatuses()` (`docs.ts`) legge la stessa funzione invece di ricopiare la lista — fail-open su fallback dichiarato se la RPC non è raggiungibile | UPDATE doc_items |
| `project_retire` | (v0.32.6/v0.32.7, SDES-DOCM-030/031/032/033/035, Ritiro progetto fase 3, mandato Achille 15/09, GTD `1061ce6e`) Ritira un **intero progetto**: itera `doc_item_retire` su ogni `doc_item` non-terminale (mai `doc_promote`, che EVAL-PG-007 ha misurato bypassare il guard a grana riga perché tocca solo `documents.status`). Due fasi: `dry_run=true` default, sola lettura, riporta ogni riga con link/xproject_link/sottoscrizioni attive in ingresso come `blocks`; `dry_run=false` ricalcola FRESCO (mai una dry-run precedente fidata) e **rifiuta in blocco** se resta anche un solo blocco di **tracciabilità** (link/xproject_link) — REQ-DOCM-025 applicato a grana progetto, mai aggirato in bulk. **Le sole sottoscrizioni attive NON bloccano l'esecuzione** (scelta esplicita, non nel disegno originale): bloccare sulla loro semplice esistenza renderebbe irraggiungibile per costruzione il ramo "ritiro riuscito notifica i sottoscrittori" di SDES-DOCM-032 — vengono invece notificate via `board_send` a esecuzione riuscita. Su esecuzione pulita: ritira ogni riga, notifica ogni sottoscrizione toccata (owner risolto via `board_agents`), chiude i GTD del progetto (`loomx_item_projects`) solo se **inequivocabilmente parcheggiati** (`someday`/`waiting`) — tutto il resto finisce in `gtd_needs_reassignment` (la riassegnazione resta loomy-only, ratifica 15/09 su `gtd_update(owner=...)`) — e **solo allora**, se zero errori di ritiro E zero `needs_reassignment`, scrive il tombstone di progetto: `loomx_projects.retired_at/retired_reason` + `status='archived'` nella STESSA update (richiesto dal CHECK `loomx_projects_retired_implies_archived`). **Schema LIVE (dba msg `9c2add8d`), GRANT in sospeso:** su `loomx_projects`, `doc_rw` e il ruolo nativo `board-mcp` hanno solo SELECT — solo loomy ha UPDATE per-colonna; questa scrittura fallisce con permission-denied finché dba non concede il grant per-colonna al ruolo che gira `project_retire` (richiesto, msg board-mcp→dba in coda). Non atomico sull'intero progetto (ogni riga è una propria transazione `doc_rw`, limite as-built dichiarato). **Lacuna dichiarata, non aggirata (D-136 §5):** i blocchi contano riferimenti in ingresso GREZZI, senza filtrare per stato del referenziante — REQ-DOCM-029/SDES-DOCM-034 propone il filtro "solo in vigore" già usato per la tracciabilità, ma cambia un parametro/trigger DB già live e serve la firma di Achille prima di essere applicato. **Lacuna dichiarata:** i Work Item intestati al progetto non sono enumerati (`wi_query` non ha scoping per `project_id`, verificato 2026-09-16, gap aperto in SDES-DOCM-035). Legittimazione: solo il responsabile del progetto (`loomx_projects.agent_id`) o loomy — stessa regola di `gtd_query(project_id)` cross-owner (DEC-002) | UPDATE doc_items×N [+ INSERT board_messages + UPDATE loomx_items + UPDATE loomx_projects] |
| `doc_query` | Filtro item + traceability (`req_without_sdes`, `sdes_without_uat`, **`req_without_origin`**, **`broken_refs`**). **`broken_refs` (v0.27.0, msg forge `162add27` / loomy `c473e347` — UAT-PG-008 passo 1, condizione 11 dell'auditor):** quarta verifica, asse **integrità dei legami**, su **entrambe** le tabelle generiche (`doc_item_links` + `doc_item_xproject_links`), sotto `doc_rw` — ogni agente misura con la **propria** identità (il `permission denied` di forge era sul suo ruolo nativo, non su `doc_rw`). Unità di analisi: il **legame**, non la riga. Tre esiti mai fusi: `items` = legami verso una riga **ritirata** (`retired_statuses` = superseded/deprecated/archived/rejected, restituito in risposta perché la regola resti ispezionabile), ciascuno con `kind` e — se l'erede è visibile — `successor_id` + il rimando a `doc_item_chain`; `abstained_items` = bersaglio illeggibile da questa identità (la policy SELECT dei legami è ancorata al **solo** estremo FROM: si legge il legame e non il bersaglio — 11/1214 e 2/283 misurati); il resto `ok`. **Il puntatore a vuoto è impossibile per costruzione** (FK `ON DELETE CASCADE` su entrambi gli estremi di entrambe le tabelle) → `dangling:{count:0, measured:false, basis}`: un fatto strutturale con la sua base dichiarata, **mai** uno zero misurato — sotto RLS «cancellato» e «nascosto» tornano identici, e spacciare quello per zero sarebbe la cecità-vestita-da-assenza vietata da D-206/REQ-DOCM-012. Gli archi `supersedes` sono **esclusi** dalla classificazione (`coverage.skipped_supersedes`): puntano dal ritirato al suo erede per costruzione, valutarli marcherebbe come difettoso ogni item correttamente versionato. `cross_project_in` è un **pavimento**, non un totale (i riferimenti in entrata da progetti illeggibili sono invisibili), e lo dichiara. Misurato dal vivo 2026-08-29: board-mcp 151 classificati/0 rotti; project-governance 724/**26** (24 `deprecated` + 2 `superseded` senza erede). Fuori copertura, dichiarato: `doc_item_gtd_links`/`doc_item_wi_links` (`doc_rw` non può leggere `loomx_items`/`loomx_work_items`, `42501` misurato — un legame verso un GTD soft-deleted resta invisibile). **`visibility_gap` (D-167, v0.16.5):** 0 righe + chiamante senza `loomx_agent_in_project` sul progetto → risposta porta `visibility_gap:true` + `note` — il conteggio nudo era indistinguibile da corpus vuoto e ha mascherato per giorni un gap-check RLS-bloccato (GTD 63142305). **Falso positivo su project_id inesistente corretto (2026-09-03, GTD 89c232d4):** `loomx_agent_in_project` ritorna `false` sia per "progetto esistente senza membership" sia per "progetto inesistente" — la nota indicava sempre "richiedi membership a dba", un'azione inutile per un id inventato. Ora `visibilityGap` prova prima l'esistenza del progetto (client service-role, passato via `DocContext.serviceDb` — `doc_rw` non ha grant su `loomx_projects`, stesso muro di `verifyProjectExists`/`docFactSync`): se non esiste, la nota lo dichiara esplicitamente senza parlare di permessi; solo se il progetto esiste ricade sul controllo di membership esistente. **`req_without_origin` (v0.22.0, D-206, GTD 1b793e87 follow-on, msg loomy 28e9aa98):** terza verifica di tracciabilità, asse UPSTREAM (requisito→origine) — distinta dalle prime due che coprono la catena a valle (req→sdes→uat). Le 4 origini ammesse da D-206: elemento capitolato (`objective`/`deliverable`/`stop_condition`), decisione cross-progetto, decisione di progetto, documento a cui si ispira (qualsiasi altro `item_type` fuori dalla catena a valle, es. `section`/`prose`). Risposta: `coverage.covered_by` spacchetta il totale per tipo di origine (mai fuso in un numero solo); un legame cross-progetto il cui `item_type` non è risolvibile (RLS-invisibile da qui) **non** conta come "nessuna origine" — finisce in `abstained_items`/`coverage.abstained`, distinto dal vero gap in `items`/`coverage.gap` (per D-206 un gap vero significa capitolato incompleto, non requisito difettoso) | SELECT + JOIN in JS |
...
| `doc_promote` | (v0.31.0, ISS-046, it-manager msg `2533b2a6`) Il verbo mancante per `documents.status`: la colonna esiste (CHECK `draft\|in_review\|approved\|active\|superseded\|archived\|deprecated`, `DB_DOCUMENT_STATUSES`) ma zero `.update()` in tutto `src/` la scriveva — `doc_publish` muove solo `version`/il ledger. **STATUS ONLY** (stessa scomposizione di `doc_rename`/`doc_publish`: title/version/status sono atti diversi con legittimazioni diverse — un `doc_update` generico è il difetto di semantica-patch già pagato in v0.16.3). Legittimazione: owner o loomy. **Nessun grafo di transizione imposto** (D-136 §5): `DOCUMENT_TYPE_REGISTRY` concede già a ogni `document_type` l'intero enum, quindi il tool rispecchia quello — qualunque valore è raggiungibile da qualunque altro; una regola d'ordine (es. `draft` non può saltare ad `archived`) è di chi possiede quella norma, non di questo server. Rilettura D-132; `old_status` catturato per valore prima della scrittura (stessa trappola di `doc_rename`); stesso valore = no-op, mai un errore. **Verificato dal vivo** (`tests/verify-doc-promote.ts`, transazione annullata) prima di dichiararlo chiuso: a differenza di `documents.version` (UPDATE revocato a `doc_rw`, solo `gov.doc_publish()` SECURITY DEFINER può scriverlo, sessione #105) lo status NON ha lo stesso muro — l'UPDATE diretto atterra, 6/6 controlli verdi | UPDATE documents.status |
| `doc_structure` | (v0.25.0, PJ-8; blocco `project {name, short_name, container_type, is_critical}` v0.33.1 — letto via client di servizio, `project_unavailable` se illeggibile) Legge la **struttura** di un progetto: documenti (tipo, titolo, stato, owner, visibility, versione dichiarata **e** pubblicazioni reali nel ledger — non sono la stessa cosa, e la differenza è il debito di pubblicazione), righe per stato e per tipo, legami per relazione, legami cross-progetto contati a parte, sottoscrizioni per origine **e** grado, marcature aperte. Chiude il gap annotato in #126/#130: `doc_query` filtra gli **item**, quindi un documento vuoto era indistinguibile da uno inesistente e nessuna superficie elencava i documenti. Uno zero dichiara **quale** zero è (`visibility.member` + nota: non-membro → «non misurabile da qui», mai «vuoto»). **Nessun fallimento tollerato:** ogni errore di lettura ritorna errore — la prima stesura degradava con una nota e, sotto `doc_rw` (una sola transazione, un errore la aborta), ha risposto «0 sottoscrizioni, 0 marcature» per un progetto che ne aveva 104 e 1 | SELECT documents/doc_items/link/gov.* |
...
| Tool | Descrizione | Operazione DB |
|---|---|---|
| `doc_subscribe` | Crea sottoscrizione origin='choice' (le origini 'fact' sono automatiche, fuori da questo tool). Esattamente uno tra `target_item_id`/`target_document_id`. Idempotente su stesso intent (`created:false`); intent diverso = cambio di grado (`intent_changed`). `critical` cross-progetto **rifiutato in v1** (D-186 Q2) — **tranne** una decisione cross del cappello, vedi gate sotto. **Gate ammissione sospesa (v0.24.3, SDES-SUB-009, REQ-SUB-013 c.3, ask SDES-SUB-CP-004 §4 — msg forge `05ba7c9e`):** legge `loomx_governance_params.sottoscrizioni_ammissione_sospesa` e mentre vale ≠0 **rifiuta** le ammissioni nuove con errore parlante. Il gate sta **dopo** i rami idempotenza/cambio-grado, di proposito: ri-chiamare su una sottoscrizione che già hai non ammette nulla, e un cambio di grado agisce su un bersaglio già ammesso — congelare anche quelli sarebbe un allargamento della regola, che appartiene a chi la governa (D-136 §5). Flag illeggibile/assente/deprecato → **ammette ma lo dichiara** (`admission_gate: "NOT VERIFIED — …"`): murare la flotta per un parametro mancante è peggio del guasto prevenuto, tacerlo farebbe passare per verificata un'ammissione che nessuno ha verificato. **Gate decisioni cross del cappello (v0.32.1, DEC-SUB-001/REQ-SUB-014, Achille 2026-09-04, supera REQ-SUB-012/SDES-SUB-012 — residenza corretta 2026-09-04, msg loomy `c99bf826`: la decisione vive nel registro del progetto Items Subscription — `DEC-SUB-001`, progetto `52f9b563-f311-4dee-8924-35145e597707`, doc `c8a09111`, item `beaec340` — non nel corpus cross del cappello; CORE-009/CORE-011: una decisione vive dove vale. `D-250` (corpus hub `22ae4e79`) resta come rimando superato):** una riga verso una decisione cross nei documenti `decisions` del progetto cappello (`22ae4e79-1800-4975-ba46-cd2f86734257`) è **sottoscrivibile** come dipendenza dichiarata (D-206 "sottoscrive, non collega") — l'obbligo della decisione vale comunque, la sottoscrizione non è mai letta come ereditarietà/adesione (le norme arrivano via D-235/CORE-012). Gradi ammessi **esattamente** `module`/`critical` (nota sempre obbligatoria); `informative` resta rifiutato (un watch leggero su una decisione cross non è una dipendenza — la ragione originale di REQ-SUB-012 resta valida a quel grado). `critical` è ammesso anche cross-progetto qui, deroga esplicita al rifiuto generale D-186 Q2 sopra. REQ-SUB-012 rimane come criterio di **identificazione del bersaglio** (project residency + `document_type='decisions'`, interinale — la regola durevole resta una CLASSE core/ambient da registro categorie, non ancora costruita, SDES-SUB-012 da emendare) | INSERT/UPDATE gov.doc_subscriptions |
...
>
> **Verificato dal vivo (non solo per lettura del codice), 2026-08-24/25, progetto board-mcp, coppia reale `SDES-DECAY-001`/`UAT-DECAY-001`:** sottoscrizione critical → riscrittura sostanziale del SDES → marcatura M2 reale rilevata → `doc_decay_apply` porta il UAT a `decayed` preservando `pass_fail` → idempotenza sul rilancio → `doc_staleness_close` chiude, ri-chiudere è no-op. Un bug reale trovato SOLO dal giro dal vivo (mai dai test unitari con fake DB): `node-pg` ritorna `timestamptz` come `Date` e `numeric` come stringa — `decay_since` andava normalizzato a ISO string prima della validazione attrs, altrimenti lo schema JSON lo rifiutava (fix in `toIsoString()`/coercizione numerica, `src/staleness.ts`). **I due anelli restanti, verificati dal vivo il 2026-09-03 (GTD `e6090f42`, WI `775e2da9`):**
- **`gov.doc_frozen_row_touches`:** creato un documento SDES probe dedicato (`[probe] Frozen Row Touch fixture`), pubblicato con un ciclo `doc_publish` reale (changelog-entry vera, prima pubblicazione mai avvenuta sul progetto board-mcp — `documents_published` era 0/10), poi riscritta una sua riga (`FRTEST-ROW-002`) fuori dall'atto di pubblicazione. `doc_staleness_query` ha riportato l'entry in `frozen_row_touches` con `doc_item_id`/`document_id`/`changed_columns`/`changed_at` corretti — il canale è vivo e letto correttamente. **Nota emersa per caso:** una riga *aggiunta dopo* la pubblicazione nello stesso documento (mai parte dello snapshot pubblicato) viene comunque classificata come frozen-row-touch alla sua prima UPDATE, non come marcatura di staleness ordinaria per i suoi sottoscrittori — il detector è a livello di documento pubblicato, non di riga-nello-snapshot. Coerente con la lettera della spec ("una riga di un documento pubblicato"), ma vale la pena saperlo prima di piazzare fixture di test in un documento già pubblicato.
- **Propagazione a due salti:** fixture A2→B→C (A2 = `sdes_entry` non pubblicato, B/C = `uat_case` critical). Riscritto A2 → marcatura aperta su B. `doc_decay_apply` ha decaduto B (`attrs.decay_status`). **Senza alcuna azione manuale**, è comparsa una nuova marcatura aperta con subscriber=C, target=B, `changed_columns:["attrs"]`, stesso timestamp della scrittura di decadimento su B — il trigger M2 è rifirato da solo sulla riga appena decaduta, confermando la cascata "per onde" descritta in `SDES-DECAY-002`.

Entrambi i fixture restano nel corpus come righe dedicate (`FRTEST-ROW-*`, `PROPTEST-*`, documento probe `[probe] Frozen Row Touch fixture`) — stesso pattern additivo già in uso per `SDES-DECAY-001`/`UAT-DECAY-001`.
...
**Implicazione (misurata 2026-08-29, GTD `8471a512`/[ISS-???]) e fix (v0.28.1, stesso GTD):** i moduli ESM di `tools.ts` si caricano via `import()` dinamico **dentro** ciascun handler, non in testa al file — quindi ogni modulo entra nella cache ESM del processo alla **prima invocazione** del tool che lo importa. Rigenerare `dist/` a finestra viva la lasciava con un **mix**: alcuni moduli dalla build nuova, altri da quella vecchia già in cache (Node non invalida mai una entry ESM già caricata quando il file cambia su disco — verificato dal vivo con un repro isolato). Osservato prima del fix: `docs.js` nuovo + `factSync.js` vecchio → `doc_link` fallito con `deriveFactOnLink is not a function` e — sotto `doc_rw`, una transazione per chiamata — **link legittimo rollbackato**. Un errore che non corrispondeva a nessuna build mai esistita.
**Fix:** `startServer()` (`src/server.ts`, `preloadLazyModules()`) precarica **eager**, in un unico `Promise.all`, tutti i moduli altrimenti importati lazy (`wi.js`, `wiCache.js`, `pendingInbox.js`, `docDb.js`, `docs.js`, `subscriptions.js`, `staleness.js`, `factSync.js`, `structure.js`) — PRIMA che il server accetti la prima tool call. Questo fissa l'intero set alla build presente su disco al boot: rigenerazioni successive non possono più produrre un mix, solo il comportamento già documentato e accettato ("la finestra resta sul build con cui è partita"). Verificato dal vivo: repro isolato del meccanismo cache-per-specifier di Node, + import diretto dei 9 moduli da `dist/` reale senza errori.
**Resta vero (non risolto da questo fix):** una finestra già aperta prima del rebuild resta sul build **precedente** finché non riavvia (comportamento G4 di base, ora l'unico possibile). Corollario operativo invariato: dopo un `npm run build` le verifiche si fanno dal **sorgente** (`runDocRw` + `tsx`), non dai tool MCP della finestra che ha lanciato la build. Nessun segnale di build per-finestra è oggi disponibile per un audit ("quali finestre sono stale") — `PACKAGE_VERSION` (package.json) non viene bumpato ad ogni commit e comunque non è esposto da Claude Code. Non forzare restart di flotta di propria iniziativa: segnalare a Loomy/it-manager, che decidono se e quando coordinare i restart (pattern usato in sessioni #63/#64/#68).
...
I `.md` in `docs/` con header `<!-- GENERATED — do not edit, source=DB -->` non vanno editati: il lint (`doc-lint.sh`) fallisce con `frozen-md-edited`. Per rigenerare il mirror: **da `hub/forge/`** (il bin non è installato fuori da quel workspace — ISS-067): `npm run doc:dump -- --document-id <uuid> --output "/home/loomy/01. Progetti/17. LoomX Home/loomx-board-mcp/docs/<file>.md"`.
