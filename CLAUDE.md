...
| `gtd_add` | Crea nuovo item GTD. **`project_id?`** (D-102, v0.16.0): UUID `loomx_projects` opzionale — valida l'esistenza (errore esplicito se non trovato, nessun GTD orfano) e crea il link `loomx_item_projects` nella stessa chiamata (`project_link` in risposta). Opzionale in questa fase, mai derivato dal WI attivo | INSERT [+ UPSERT loomx_item_projects se project_id] |
